...
export default function Menu(props) {
	...
	function renderSocialIcons(data) {
		return (data.map(curIcon => (<a href="/" tabIndex={!props.menuOpen ? -1 : 0} className="link-2" key={curIcon.key}>
	...
	function renderLangs(data) {
		return (data.map(curLang => (
			<a href="/" tabIndex={!props.menuOpen ? -1 : 0} className={`link-1 ${langActive[curLang.id] ? "text-neutral-3" : ""}`} id={curLang.id} key={curLang.key}>
				<span className="sr-only">go to</span>{curLang.content}<span className="sr-only">page</span>
			</a>)))
	}

	function renderList(data) {
		return (data.map((curItem, i) => (<li className="menu__item" key={curItem.key}>
			<a className="link-2 link-underline letterspacing-4" tabIndex={!props.menuOpen ? -1 : 0} href="/">
				{curItem.link}
